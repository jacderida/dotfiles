function rust-ec2-instance
    test ! -d ~/.aws && mkdir -p ~/.aws
    rm -f ~/.aws/quick-instance-id

    set image_id "ami-0cd8ad123effa531a" # Amazon Linux base image
    set keypair $AWS_QUICK_EC2_INSTANCE_KEYPAIR
    set instance_type "t2.xlarge"
    ...
    set public_ip (aws ec2 describe-instances \
        ...
        --output text)
    ssh -oStrictHostKeyChecking=no ec2-user@$public_ip "bash --version"
    ...
    printf "%s\n" \
        ...
        | ssh -oStrictHostKeyChecking=no ec2-user@$public_ip

    echo "ssh to the instance with: ssh ec2-user@$public_ip"
    echo "ssh ec2-user@$public_ip" | wl-copy
end
